...
class InferenceDisentangledSelfAttention(OriginalDisentangledSelfAttention):
    ...
    def prepare_for_inference(
        ...
        *,
        ...
        """Cache layer-dependent relative projections and optional fused QKV."""

        if self.training:
            raise RuntimeError("prepare_for_inference() requires module.eval()")

        self.clear_inference_cache()
        use_fused_qkv = self.fuse_qkv if fuse_qkv is None else fuse_qkv
        self.fuse_qkv = use_fused_qkv
        if use_fused_qkv:
    ...
    def _project_qkv(
        ...
        if self._cached_qkv_weight is not None:
            projected = F.linear(
                hidden_states,
    ...
    def forward(
        ...
        needs_qkv = self.fuse_qkv and self._cached_qkv_weight is None
